...
void debug();
void printSlopes(std::vector<Point> &points);
...
//#define DEBUG
			...
			if (Point::areCollinear(v[i], c[cur], c[cur -1])) {
				...
				/* end of collinear segments and 3 or more points found
				 * on top of ref point */
				if (count >= 3) {
					/* not inside yet, so can print */
					points.clear();
