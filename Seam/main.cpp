...
double getMin(int i, int j) {
	...
	min[i][j].path = std::make_pair(mi, mj);
...
int main(int argc, char *argv[]) {
	...
	for(int i = 0; i < ROWS; i++) {
		for(int j = 0; j < COLS; j++) {
			...
			min[i][j].path = std::make_pair(-1, -1);
		}
	}
	int k = std::stoi(argv[1]);
	int h = std::stoi(argv[2]);
	double minVal = getMin(k, h);
	std::cout << "min: " << minVal << std::endl;

	int cur_i = k, cur_j = h;

	while(cur_i >= 0) {
		std::cout << "(" << cur_i << ", " << cur_j << ")" << std::endl;
		std::pair<int, int> path = min[cur_i][cur_j].path;
		cur_i = path.first;
		cur_j = path.second;
	}
